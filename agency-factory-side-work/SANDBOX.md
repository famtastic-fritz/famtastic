...
This directory (`agency-factory-side-work/`) is a **self-contained sandbox**. The rule is
simple and absolute:

> **Nothing outside `agency-factory-side-work/` may be modified by this system — ever.**
...
|--------------------|---------------------------------------------------------------------------|
| Code               | All `.py` files live in `agency-factory-side-work/`.                                  |
| State / queue      | SQLite DB at `agency-factory-side-work/data/factory.db`.                             |
| Logs               | `agency-factory-side-work/logs/ORCHESTRATOR.log`, `agency-factory-side-work/logs/COSTS.log`.    |
| Scheduling         | In-process scheduler thread. **The real OS crontab is never touched.**     |
| Spawned agents     | Minted into `agency-factory-side-work/agents/`, run as local subprocesses.            |
| Deliverables       | Written to `agency-factory-side-work/deliverables/`.                                  |
| Dashboard          | `agency-factory-side-work/dashboard/index.html` (static) + terminal readout.         |
| Secrets            | Read from `agency-factory-side-work/.env` only (you create it; `.env.example` shows it).|
...
The original brief said "create `./agency-factory-side-work/` with its own git repo." A
