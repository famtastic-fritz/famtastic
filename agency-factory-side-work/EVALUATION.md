...
- **The branch itself: safe.** `claude/agent-factory-orchestrator-flvu43` only
  *adds* `agency-factory-side-work/` (plus two `obsidian/` session notes). It modifies zero
...
```bash
# from a checkout/copy of agency-factory-side-work/ (any location)
...
make eval               # run + copy artifacts to /tmp/agency-factory-side-work-eval for inspection
...
to `OUT=/tmp/agency-factory-side-work-eval` (outside your repo) so you can read results
...
```bash
make workspace DEST=~/agency-factory-side-work-eval   # copies the factory out of the repo
cd ~/agency-factory-side-work-eval
python3 run.py                              # or: ./demo.sh autonomy
```
This writes only under `~/agency-factory-side-work-eval`. Your `~/famtastic` (and every
...
cd ../famtastic-af/agency-factory-side-work && python3 run.py
