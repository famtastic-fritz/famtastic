...
**Last updated:** 2026-08-01 (FAMtastic Designs now dispatches signed, idempotent three-proof jobs to Site Studio; Site Studio generates through Shay, returns portable script-free artifacts by signed callback, and FAMtastic Designs blocks placeholder proofs from customer outreach.)
...
- **2026-08-01** — FAMtastic Designs real proof integration. Added the HMAC-authenticated `/api/integrations/famtastic/proof-jobs` machine route, durable job/idempotency/restart state, exactly-three direction mapping, Shay-based generation, portable artifact packaging, and signed callback retry. Corrected the public proof base from `/web/proofs` to `/proofs` and proved the customer path with three browser-rendered Copper Kettle Bakehouse designs. This is locally proven and Git-closeout ready; production environment configuration/deployment remains a separate authorized operation.
- **2026-07-23** — Site Studio runtime-vnext modular cutover baseline. Promoted the current runtime-vnext lane into the new local base candidate after extracting named seams out of the old all-in-one Studio shell: `site-studio/server/spec-store.js`, `server/runtime-status-routes.js`, `server/deploy-repo-routes.js`, `server/studio-state-routes.js`, `server/ws-runtime.js`, `runtime-vnext/register-routes.js`, and `runtime-vnext/lib/page-output-path.js`, plus modular frontend panel/runtime scripts under `site-studio/public/js/`. Frozen the external contract in `site-studio/runtime-vnext/SITE-STUDIO-CALLABLE-CONTRACT-2026-07-23.md` and closed the milestone brief in `SITE-STUDIO-RUN-TO-COMPLETION-BRIEF-2026-07-23.md`. Verified with a live Studio on `http://127.0.0.1:3334/`, `GET /api/health` = 200, 38/38 passing runtime-vnext characterization/seam/contract/server-bridge/milestone-E tests, and on-disk proof artifacts for the green JJ B&A Transport rebuild. Honest gap: `site-studio/server.js` is still a large backend monolith, so the baseline is modular-and-callable, not fully decomposed.
- **2026-07-22** — Site Studio Proof Mode endpoints. Added `POST /api/proof-generate` and `POST /api/proof-campaign` in `site-studio/server.js` so pre-sale proof directions can generate isolated `_template.html`, `index.html`, and `design-dna.json` artifacts without `/api/new-site`, active `TAG` mutation, `finishParallelBuild()`, `runPostProcessing()`, Netlify deploy, full verification, or image/video autofill. Campaign proofs run sequentially with per-direction layout/font/color/density overrides, and thumbnail generation is available only when a hosted `proof_url` is supplied. Verified with `node --check site-studio/server.js`, localhost route validation on throwaway ports, `git diff --check -- site-studio/server.js`, and `node scripts/plans/audit.js`.
- **2026-07-22** — MBSH backend recovery + live poll lane. Recovered the production PHP backend into `sites/site-mbsh-reunion/backend/`, ending the server-only drift from the dead `~/famtastic-sites/mbsh-reunion` path. Added `backend/schema.sql` with `poll_questions`, `poll_options`, and `poll_votes`; added `backend/poll.php` for the public voting surface; added `backend/admin/polls.php` plus the `admin/dashboard.php` poll link; and updated `sites/site-mbsh-reunion/spec.json` so the canonical tracked repo path now points at `~/famtastic/sites/site-mbsh-reunion`. Verified by applying the schema live on prod, deploying the backend through `platform/capabilities/deploy/deploy-backend.sh`, loading `https://api.mbsh96reunion.com/poll.php`, submitting a live vote, and confirming the vote landed in the production database. Remaining honest gap: the vault-stored admin password did not successfully authenticate against `/admin/login.php`, so the admin auth lane still needs a credential/throttle cleanup pass.
- **2026-07-02** — Shay autonomous enhancement closure pass. Added `shay-shay/agent/reviewer_routing_guard.py` and wired `shay-shay/tools/delegate_tool.py` to block protected adversarial/final-blocker reviewer lanes when the selected or inherited reviewer lacks promotion evidence in `~/.shay/runtime/model-routing/reviewer-summary.json`; every decision is logged to `~/.shay/runtime/model-routing/route-decisions.jsonl`. Added prefetch proof traces in `shay-shay/agent/memory_manager.py` at `~/.shay/runtime/prefetch/prefetch-proof.jsonl`, plus `AuxiliaryLLMReflectionClient` in `shay-shay/agent/intelligence_governance.py` so `shay-shay/agent/intelligence_loop.py` can run real aux-LLM generative reflection when enabled instead of always falling back to local synthesis. Verified with `uv run pytest tests/agent/test_reviewer_routing_guard.py tests/agent/test_memory_prefetch_trace.py tests/agent/test_reviewer_bakeoff.py tests/agent/test_intelligence_loop.py -q` (18 passed).
- **2026-07-02** — Reviewer route bake-off + telemetry validation. Added `shay-shay/agent/reviewer_bakeoff.py`, seeded five benchmark packets at `shay-shay/docs/benchmarks/reviewer-bakeoff-packets.json`, and added regression coverage in `shay-shay/tests/agent/test_reviewer_bakeoff.py` so reviewer model fit is now scored with packet hashes, artifact-consumption gates, citations, severity ranking, actionable issue counts, seeded issue markers, JSONL scorecards, raw live-output artifacts, and reducer recommendations. `shay-shay/agent/swarm_telemetry.py` now validates lane timestamp ordering before JSONL writes, preserves manually logged durations, records observed timestamp-derived duration, and flags invalid clocks through `telemetry_invalid_time`. Verified deterministic scoring plus live provider probes: `openai-codex/gpt-5.5` passed 5/5 seeded reviewer packets and was recommended for tested task-family promotion, local `custom:ollama-local/qwen3:14b` passed 3/5 and remains candidate/limited-review, and mislabelled `custom/glm-5.1`/`custom/gemma4:latest` probes truthfully failed as provider 404s. Deferred: runtime enforcement in `delegate_task` / routing controller consumption of the scorecard summary.
- **2026-07-02** — Shay intelligence-loop five-slice closure. Added `shay-shay/agent/intelligence_prefetch.py`, wired it into `shay-shay/run_agent.py`, and added `shay-shay/agent/intelligence_loop.py` so the live agent can keep `MEMORY.md` / `USER.md` small while dereferencing pointer artifacts and current intelligence state into an ephemeral per-turn context block. The loop now produces local L2/L3 reflection synthesis, review-gated pointer candidates, review-gated capability reconciliation proposals, and session-context candidates under `obsidian/Shay-Memory/reflections/`, and the prefetcher now loads those generated artifacts when relevant. Verified in the isolated `~/famtastic-worktrees/shay-intelligence-loop` worktree with 165 passing focused tests plus a live artifact run; deferred: paid/deep auxiliary-LLM reflection, silent canonical memory mutation, silent capability-matrix mutation, and permissioned ambient integrations such as calendar/iMessage/browser/screen state.
- **2026-06-25** — Shared route-truth runtime enforcement + Codex opt-in cleanup. `shay-shay/shay_cli/intelligence_control_plane.py` now emits universal route metadata (`generated_at`, `source_files`, `refresh_command`) and strips Codex out of default review/browser/orchestrator preference lists while keeping both `openai-codex-gpt-5.4` and `openai-codex-gpt-5.5` blocked wherever the anti-drain builder policy applies. `shay-agent-os/components/swarm/brain_client.py`, `asyncio_dispatcher.py`, and `local_swarm_dispatcher.py` now refresh the shared export when the Shay control-plane sources are newer, pass `task_family` into live calls, and enforce shared task-family policy at runtime instead of only using local brain ordering. Verified with `uv run pytest tests/test_intelligence_layer.py -q` (74 passed), live `shay intelligence control-plane export`, direct JSON inspection of the exported truth, and direct `brain_client` import checks showing implementation resolves to `claude -> openrouter -> gemini -> ollama` while review resolves to `gemini -> claude -> openrouter -> ollama`. Deferred: there is still no true always-on file watcher/git hook for export publication, and `shay-agent-os` still does not consume the full provider/model metadata as a budget-aware scheduler.
- **2026-06-24** — Shay live model-probe + capability registry routing hooks. Added `shay-shay/shay_cli/model_probe.py` as a new control-plane truth surface that writes persisted probe results to `~/.shay/runtime/model_capability_registry.json`, merges effective capability truth in the order `live_probe > local override > catalog claim > provider docs`, classifies billing type, and runs cheap checks for text, tool-call, structured-output, image, and long-context acceptance on API-key OpenAI-compatible lanes. `shay-shay/shay_cli/main.py` now exposes `shay intelligence control-plane probe-model <provider> <model>` and `probe-registry`, while `shay-shay/shay_cli/intelligence_control_plane.py` now uses initial probe-backed route scoring to re-rank template-preferred lanes by capability truth instead of static order alone. Verified with `.venv/bin/pytest -q tests/shay_cli/test_model_probe.py tests/shay_cli/test_zai_glm52_capabilities.py` (6 passed) plus live CLI runs for `probe-model`, `probe-registry`, and `intelligence control-plane explain`; deferred: OAuth/non-compatible providers still truthfully degrade to `skipped` probes, and the scorer is an initial hook rather than a full dynamic worker-pool scheduler.
- **2026-07-02** — FAMtastic By the Numbers public Netlify web-first hardening. Added `apps/famtastic-by-the-numbers/netlify.toml` plus `apps/famtastic-by-the-numbers/netlify/functions/api.js` so the static-hosted public deploy now exposes the premium/status/config API surface through Netlify Functions while reusing `lib/config.js`, `lib/paypal.js`, and `lib/db.js`. `lib/db.js` now detects serverless runtime and uses `/tmp/famtastic-by-the-numbers-dev-store.json` for proof-mode persistence, while `chart.html`, `app.js`, and `styles.css` were tightened with a `#intake` CTA, visible empty-state prompt, `aria-live` results, UTF-8-safe share payloads, UTC personal-year calculation, safer API error handling, less internal copy, and a narrower centered mobile-first chart shell. Verified with `npm run smoke:local`, `git diff --check`, live public HTTP checks for all six `/api/*` routes, and a live Playwright run covering generate -> compatibility -> premium unlock -> restore with no mobile horizontal overflow.
- **2026-06-25** — FAMtastic By the Numbers live PayPal proof complete + mobile hero badge fix. Updated `apps/famtastic-by-the-numbers/.env` to load real PayPal credentials from the verified hosting repo source, switched `PREMIUM_PRICE_CENTS` / `EXPECTED_PRICE_CENTS` to `100`, updated `.env.closeout.example`, and verified live truth so `/api/health` reports `paymentMode=live`, `persistenceMode=mysql`, and `premiumPriceCents=100`. Added verified live coverage with `npm run verify-paypal-config`, `curl /api/config`, a non-capturing `POST /api/paypal/create-order`, Fritz's successful real $1 PayPal purchase, confirmed return-to-app behavior, confirmed reload/restore persistence, and a mobile `styles.css` fix that stops the `Soul Urge` badge from overlapping hero copy on smaller layouts.
- **2026-06-24** — FAMtastic By the Numbers MySQL closeout proof lane. Expanded `apps/famtastic-by-the-numbers/` with `scripts/bootstrap-local-mysql.sh`, `scripts/verify-db.js`, `.env.closeout.example`, `tests/smoke-local-proof.mjs`, and `OPERATOR-CLOSEOUT-CHECKLIST.md`, while rewriting `tests/smoke.mjs` as the MySQL-backed closeout proof harness and teaching `lib/config.js` to derive the premium CTA from the configured price instead of a stale hardcoded `$22`. Verified with live MySQL bootstrap/migrate/verify, `curl` checks against `/api/health` and `/api/config`, and passing `npm run smoke` plus `npm run smoke:local` on port `4174`. Deferred: the only remaining web-proof step is Fritz inserting real PayPal credentials and making one live 2-cent browser purchase.
- **2026-06-24** — FAMtastic By the Numbers first working web app. Added `apps/famtastic-by-the-numbers/` as a new web-first app surface with `index.html`, `styles.css`, `app.js`, `README.md`, `tests/smoke.mjs`, and `GAPS.md`. The build is a transparent Pythagorean V1 numerology recognition system: free core chart, visible math breakdown, Life Path / Expression / Soul Urge / Personality / Birthday / Personal Year, lightweight compatibility read, daily guidance, local save/share, and explicit trust language in-product. Wave 1 evidence and product-shape artifacts now live under `obsidian/01-Shay-Platform/famtastic-by-the-numbers-wave1/` (`competitor-matrix.md`, `lineage-decision-memo.md`, `v1-schema.md`, `trust-monetization-memo.md`). Verified with `node --check`, local `python3 -m http.server 4173`, Playwright smoke test, and browser console check; deferred: payment rail, cloud persistence, deeper compatibility, Chaldean mode, and the HyperSwarm/web-search routing gaps that required captain-direct recovery.
- **2026-06-23** — Shay ask-trace CLI + HyperSwarm internal-lane truth alignment. Added `shay-shay/shay_cli/intelligence_cmd.py` ask tracing via `ASK_TRACE_RULES`, `trace_task()`, and `format_trace()`, registered `shay intelligence trace <ask>` in `shay-shay/shay_cli/main.py`, and documented the canonical mapping in `shay-shay/docs/ask_intent_registry.md`. The slice also reclassified HyperSwarm internal production lanes from falsely blocked to enabled-with-controls in the live CLI truth surfaces, added tests/worked examples for build, attention, GitHub-to-Obsidian, and context-compression asks, and repaired the live `~/.shay/memories/USER.md` authority drift so trace runs no longer emit false identity alarms. Deferred: the natural-language intent map is still thin for competitor research, chained `research -> build prototype`, and pre-build swarm preview/confidence asks; those still require procedural commands today.
- **2026-06-22** — Conversation freeze capture lane. Added `scripts/conversation-freeze.js` so bounded conversation lanes can first freeze a canonical source artifact into `obsidian/05-Captures/freezes/<date>/<freeze-id>/` with `freeze-manifest.json`, `transcript.md`, `transcript.json`, and an index row in `obsidian/05-Captures/index/freezes.jsonl`. Upgraded `scripts/conversation-export.js` to accept `--freeze-manifest`, preserve frozen anchor fields in `export.json`, and keep the file-first export package intact while shifting the seed from a saved brief surrogate to the frozen transcript when available. Verified with live proof artifacts at `obsidian/05-Captures/freezes/2026-06-22/frz_20260622_71490bfe/`, `obsidian/05-Captures/exports/2026-06-22/exp_20260622_8fcb50fb/`, and `obsidian/05-Captures/review/convo-freeze-proof-2026-06-22.md`. Deferred: current anchors are derived from normalized saved-artifact blocks rather than native provider message IDs, and there is still no dedicated replay/query CLI over the freeze/export indexes.
- **2026-06-18** — Scram-line brief default. Changed `plans/templates/simple-brief-template-v1.md` from the more verbose simple-brief shape into the default ultra-brief resumable format: `Title`, `Purpose`, `Goal`, checkbox `Tasks`, `Status`, `Started`, `Ended`, `Execution`, `Research`, `Review`, `Skills`, optional `Blocked By`, and `Proof`, with exact spacing rules so the brief stays consistent and scannable. Mirrored the same rule into `docs/agent-startup/AGENT-STARTUP-CONTRACT.md`, `AGENTS.md`, and `CLAUDE.md`, and made the heavier control-plane plan packet explicitly secondary to the human-facing scram-line brief. Deferred: there is still no `fam-hub` command or dedicated validator for the scram-line shape.
- **2026-06-17** — Simple brief template v1. Added `plans/templates/simple-brief-template-v1.md` as a lightweight human-brief surface that stays readable while still showing orchestration truth. The template originally carried `Execution` plus task-mapped `Assignments` (task → agent → model), artifact-backed `Research`/`Review`, dependency plan IDs, proof, and status, with both a blank form and a filled example. This shape has now been superseded as the default by the leaner scram-line brief above.
- **2026-06-17** — Shay Desktop/Desktop shortcut/Workspace runtime cutover. Both `/Applications/Shay Desktop.app` and `/Users/famtasticfritz/Desktop/Shay Desktop.app` now launch against `/Users/famtasticfritz/.shay` through the live `/Users/famtasticfritz/.local/bin/shay` runtime sourced from `famtastic/shay-shay`, and the desktop-hosted dashboard now lives on `http://127.0.0.1:9120`. Shay Workspace was rewired to the same runtime by targeting the shared gateway at `http://127.0.0.1:8642/health` and the Desktop-hosted dashboard with a fixed local token, then patched so its dashboard health checks send that token. Canonical truth for this cutover lives in `shay-shay/docs/shay-surface-map-2026-06-17.md`; visible Hermes branding remains an honest gap.
- **2026-06-17** — Plan autonomy doctrine + resumable task-state rule. Extended the plan control plane so the canonical packet now carries explicit execution policy instead of leaving autonomy implicit. `plans/plan-template.json` now requires an `execution_policy` block (`run_to_completion_without_fritz`, `intervention_threshold`, `orchestration_mode`, `swarm_expectation`, `task_ledger_expectation`), and `scripts/plans/validate-template.js` now enforces those fields. The same rule now lives in `docs/agent-startup/AGENT-STARTUP-CONTRACT.md`, `AGENTS.md`, and `CLAUDE.md`: plans should run to completion without Fritz intervention by default, multi-swarm / parallel orchestrated swarms are preferred when dependencies permit, and task state must be updated on every completion/block/change boundary so interrupted work is resumable. Deferred: the ledger still depends on agents actually writing those state changes; there is not yet an automatic reconciler.
- **2026-06-17** — Plan template enforcement + studio-filtered task surface. Added `plans/plan-template.json` as the canonical plan packet shape, added `scripts/plans/validate-template.js`, and exposed both through `fam-hub plan template` and `fam-hub plan validate` so the work-packet doctrine can be checked instead of merely requested. Replaced the old inline task renderer with `scripts/plans/task-list.js`, which now joins `tasks/tasks.jsonl` against `plans/registry.json` labels and supports `fam-hub task list --studio <value>`, `--stream <value>`, `--status <value>`, `--runner <value>`, `--compact`, and `--json`. Verified with live `fam-hub` runs; current honest gap is that `plan validate --all` now exposes legacy plan files that still need migration to the new template.
- **2026-06-17** — Obsidian memory boundary cleanup + deterministic Shay mirrors. Hardened the bridge between repo truth surfaces, `.wolf` lessons, and the shared Obsidian vault so the repo stops getting dirty from timestamp-only mirror churn. `shay-agent-os/sync_lessons_to_shay.py` now writes deterministic mirror headers, skips file rewrites when the mirrored body is unchanged, and `.gitignore` now treats `obsidian/Shay-Memory/lessons-mirror/`, `reflections/`, `builds/`, `inbox/`, and `scratch/` as local runtime exhaust rather than canonical source. This preserves `obsidian/Shay-Memory/repo-docs/` as the tracked/shared mirror surface while moving the high-churn memory buckets back to local-vault status.
...
- **2026-05-04** — Plan registry CLI substrate. Added `plans/registry.json` as the pilot canonical plan registry, three density contracts under `plans/templates/`, later expanded with `plans/templates/simple-brief-template-v1.md` for the lightweight human-brief surface, empty append-only ledgers for tasks/runs/jobs/proof/research, `FAMTASTIC-STATUS.md` for web/phone mirrors, and `docs/plan-registry-build-report-2026-05-04.md`. `scripts/fam-hub` gained read-only `plan`, `task`, and `run` status commands: `fam-hub plan list`, `fam-hub plan list --compact`, `fam-hub plan list --json`, `fam-hub plan show <id>`, `fam-hub task list`, and `fam-hub run status`. The later four-plan registry milestone added task promotion, run creation, and ledger records; automatic status export, schema validation, and Studio Plans panel rendering remain deferred.
...
| Backend | Node.js + Express 4.21 | HTTP server, REST API, WebSocket. `site-studio/server.js` is still the primary bootstrap/legacy shell, but runtime-vnext and Studio responsibilities are now partially extracted into `server/spec-store.js`, `server/runtime-status-routes.js`, `server/deploy-repo-routes.js`, `server/studio-state-routes.js`, `server/ws-runtime.js`, and `runtime-vnext/register-routes.js`. |
| Frontend | HTML shell + modular browser scripts | `site-studio/public/index.html` is still the Studio shell, but major panel/runtime behavior now lives in modular `public/js/` files such as `studio-build-controls.js`, `studio-chat-ui.js`, `studio-deploy-pane.js`, `studio-preview-pages.js`, `studio-project-switcher.js`, `studio-runtime-ws.js`, and `studio-state-snapshot.js`. CSS remains under `public/css/`. |
...
| Testing | Vitest 4.1.1 | Historical baseline suites still exist, and the 2026-07-23 runtime-vnext cutover proof added focused passing coverage for `tests/characterization-harness.test.js`, `tests/rewrite-seams.test.js`, `tests/runtime-vnext/consumer-contract.test.js`, `tests/runtime-vnext/server-bridge.test.js`, and `tests/runtime-vnext/milestone-e.test.js`, with 20/20 passing in the main cutover verification run. Legacy node-script test suites still exist in `tests/` and are not the current gate. |
...
## 2026-07-02 state update — Shay intelligence autonomy
- `shay-shay` now includes governed intelligence-loop autonomy helpers: `agent/intelligence_governance.py`, `agent/ambient_context.py`, and `agent/swarm_telemetry.py`.
- `agent/intelligence_loop.py::run_intelligence_loop_slices()` emits additional governed artifacts: `generative-reflection.json`, `pointer-promotions.json`, `capability-review-queue.jsonl`, and `ambient-context.json`.
- Safety posture: default deterministic fallback, no silent canonical memory mutation, no direct `intelligence_seed.py` mutation, ambient read-only/local by default.
- Proof: focused pytest and compileall passed on 2026-07-02; run telemetry lives at `~/.shay/runtime/swarm-runs/intelligence-autonomy-20260702T114438/`.
- Remaining gaps: auxiliary LLM provider wiring, external ambient connectors, and concrete token/cost telemetry depend on runtime/provider support.

- **2026-07-02** — FAMtastic By the Numbers Pass 1 web completion. The app under `apps/famtastic-by-the-numbers/` now includes web-facing items previously parked for later: method/privacy surfaces, Y/master-number disclosure, monthly/day guidance, composite compatibility with repair mapping, local reading history, encoded share-link loading, and updated Playwright smoke assertions. Verified with `npm run smoke:local` and `npm run smoke`; only PWA/native/app-store work remains deferred.
