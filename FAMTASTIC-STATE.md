...
**Last updated:** 2026-05-04 (Workbench Shay context provider, site workflow contracts, and MBSH backend inventory added to the four-plan execution substrate.)
...
| MBSH runtime endpoint execution | Tier 1 | Backend inventory is complete, but local endpoint execution is blocked until `.env`, `.mbsh-config.local.php`, or `MBSH_CONFIG_PATH` provides DB, Resend, admin hash, CORS, and upload-path config. Production deploy proof must also set `API_BASE_URL` away from `null`. |
