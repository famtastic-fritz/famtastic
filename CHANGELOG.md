## 2026-06-02 — Autopilot: live client email send (Resend) wired

Wired live email sending for the client-upsell agent so offer emails actually go out the moment a from-address + Resend key are present — no further code changes. New `autopilot/lib/email.mjs` reuses FAMtastic's existing Resend setup (vault `studio.resend.api_key` / `RESEND_API_KEY`, `POST https://api.resend.com/emails`, verified sender `send.mbsh96reunion.com`): `hasEmailCreds()`, `defaultSender()`, `buildPayload()`, `sendEmail()` (never throws — a send failure is recorded, not crashed). `client-upsell.mjs` now resolves a recipient by `email_mode`: default `to_operator` sends drafts to you for one-click review/forward (safe, needs no client address), `to_client` emails businesses directly when a contact is known (`extractClientEmail()` checks `client_contacts` override → spec contact fields → any email in the spec). Live send fires only when governance is live AND a Resend key AND a recipient exist; otherwise it stages the `email.txt` draft as before. New config: `email_mode`, `operator_email`, `client_contacts`. `vault.mjs` CRED_MAP gained `resend`. Verified: staged path unchanged; the live path was exercised with a fake key + `--live` and correctly recorded `send_failed` without crashing. Tests now 13/13 (added email payload/creds/recipient cases). Plan task `ws_client_email_send` → done.
