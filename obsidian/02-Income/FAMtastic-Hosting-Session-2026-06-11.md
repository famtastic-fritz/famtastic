...
## Session Continuation — Wave 2 (same session, context overflow)

**Commit:** `44ac8d0`
**What Fritz asked for:** logout button, customer number display, PayPal Add to Cart test, no GoDaddy verbiage anywhere

---

### Honest Answers to Fritz's Questions

| Question | Answer |
|----------|--------|
| Does registration send an email? | **No.** API creates account + session, no email sent. SMTP/Resend not wired. New feature to build. |
| Does forgot password work? | **No.** No page, no API, doesn't exist. New feature to build. |
| Is there a logout button? | **Fixed.** Added to sidebar — see below. |
...
#### 1. Logout Button
- **Where:** `src/components/dashboard/DashboardNav.svelte` — sidebar footer
- **How:** Red-bordered "Sign out" button. On click: `fetch('/api/auth/logout', {method:'POST'})` → `window.location.href = '/dashboard/login'`
- **API:** `POST /api/auth/logout` already existed. Just needed UI.

#### 2. Customer Number Display
- **Where:** Topbar (purple `#00001` chip) + sidebar footer (next to role label)
- **Format:** `String(userId).padStart(5, '0')` → zero-padded 5-digit ID
- **Chain:** `user.id` (string from middleware) → `Number(user.id)` in every dashboard/admin page → `userId` prop on `DashboardLayout` → `PageShell` → `DashboardNav`
- **Files changed:** `DashboardLayout.astro`, `PageShell.svelte`, `DashboardNav.svelte`, all 6 dashboard pages, all 8 admin pages

#### 3. Add to Cart Buttons — All Product Pages Confirmed
All product pages now have working Add to Cart buttons wired to correct DB product IDs:

| Page | Product | ID |
|------|---------|-----|
| wordpress.astro | Managed WP Basic | 1 |
| wordpress.astro | Managed WP Ultimate | 2 |
| hosting.astro | cPanel Starter | 3 |
| hosting.astro | cPanel Ultimate | 4 |
| builder.astro | Builder Essential | 5 ← **added this wave** |
| builder.astro | Builder Commerce | 6 ← **added this wave** |
| servers.astro | Web Hosting Plus Launch | 7 |
| servers.astro | Web Hosting Plus Expand | 8 |
| domains.astro | .com | 9 |
| domains.astro | .net | 10 |
| domains.astro | .org | 11 |
| domains.astro | .co | 12 |

Live test confirmed: `POST /api/cart/add` → `{"name":"cPanel Starter","retail_price_cents":700,"count":1,"subtotalUSD":"$7.00"}`

#### 4. GoDaddy Verbiage — Full Sweep
Removed `(480) 624-2500` phone number and "GoDaddy" name from ALL customer-visible surfaces:

**Public marketing pages (intentionally kept):**
- "No GoDaddy branding" ticker/copy — this is a selling point, kept.

**Removed from:**
- `index.astro` — "Call (480) 624-2500" in how-it-works → `/contact`
- `hosting.astro` — Talk to a Human hero CTA, phone in plan section, mailto CTA
- `wordpress.astro` — two phone numbers in FAQ
- `servers.astro` — Talk to a Human CTA, two FAQ phone numbers
- `builder.astro` — two inline phone mentions
- `bundles.astro` — Talk to a Human CTA, two FAQ phone mentions
- `domains.astro` — FAQ phone number
- `dashboard/index.astro` — "Manage via GoDaddy", dead store.famtastichosting.com quick-action links, fetch error phone
- `dashboard/billing.astro` — "All payments processed by GoDaddy", "Manage on GoDaddy" button → `/dashboard/support`
- `dashboard/email.astro` — "GoDaddy's Workspace interface" → generic
- `admin/provisioning.astro` — "GoDaddy reseller panel" reference

**Intentionally kept (appropriate context):**
- `contact.astro` — phone stays on the contact page (that's what it's for)
- `thank-you.astro` — post-GoDaddy-purchase flow page (old flow, still reachable)
- `dashboard/support.astro` — support phone for logged-in users (help section)
- Admin API code comments — internal, never user-visible

---

### Deployment Gotcha — node_modules Missing

**Problem:** After this deploy, Node failed with `ERR_MODULE_NOT_FOUND: Cannot find package 'cookie'`. The server's `site/` directory only had `.env` and `dist/`. No `node_modules`, no `package.json`.

**Root cause:** The deploy script only rsyncs `dist/client/` and `dist/server/`. It never copies `node_modules`. Previous sessions must have had a full project tree at some point.

**Fix applied:**
1. `scp package.json` and `package-lock.json` to `/home/nineoo/public_html/famtastichosting.com/site/`
2. Ran `npm ci --omit=dev` on server via `/bin/sh` script
3. Restarted Node → `[@astrojs/node] Server listening on http://127.0.0.1:3001`

**Add to deploy checklist:** After any server rebuild, check if `node_modules` exists at `site/`. If missing, run `npm ci --omit=dev` before restarting Node. Alternatively, add this to the restart script as a one-time safety check.
...
# 2. Patch entry.mjs (4 patches — required every build)
sed -i '' 's|"client": "file:///Users/famtasticfritz[^"]*dist/client/"|"client": "file:///home/nineoo/public_html/famtastichosting.com/site/dist/client/"|' dist/server/entry.mjs
sed -i '' 's|"server": "file:///Users/famtasticfritz[^"]*dist/server/"|"server": "file:///home/nineoo/public_html/famtastichosting.com/site/dist/server/"|' dist/server/entry.mjs
sed -i '' 's|"host": false|"host": "127.0.0.1"|' dist/server/entry.mjs
sed -i '' 's|"port": 4321|"port": 3001|' dist/server/entry.mjs
...
# 5. If node_modules missing on server (check first time after server rebuild):
# scp package.json package-lock.json nineoo@...:/home/nineoo/public_html/famtastichosting.com/site/
# ssh + npm ci --omit=dev in /bin/sh script

# 6. Restart Node
scp restart-node.sh nineoo@...:/tmp/ && ssh ... "/bin/sh /tmp/restart-node.sh"
```

---

### What Shay Needs to Pick Up

#### Immediate: PayPal Sandbox Browser Test
The cart and checkout are live. Shay needs to do a full browser click-through:

1. Go to `https://famtastichosting.com/hosting` (or any product page)
2. Click "+ Add to Cart" on a plan
3. Navigate to `/checkout`
4. Verify PayPal button renders (loaded from PayPal JS SDK with `client-id` from env)
5. Click PayPal button → sandbox popup should open
6. Log in with PayPal sandbox buyer account → approve payment
7. Verify redirect to `/order-confirmation`
8. Verify `POST /api/checkout/capture-order` fires and order lands in DB

**PayPal sandbox credentials:** In `.env` as `PAYPAL_CLIENT_ID` / `PAYPAL_SECRET` / `PAYPAL_ENV=sandbox`. Fritz has the sandbox buyer account credentials.

**If PayPal button doesn't appear:** Check browser console for SDK load errors. The SDK loads as: `https://www.paypal.com/sdk/js?client-id={clientId}&currency=USD`

#### Not Built — Need Building
These were explicitly asked about but don't exist yet:

1. **Registration welcome email** — No SMTP/Resend wiring. When user registers at `/dashboard/register`, they get logged in but receive no email. Build: wire `SMTP_*` or `RESEND_*` env vars (both in `.env`) to send a welcome email from the register API.

2. **Forgot password flow** — No page, no API, nothing. Build:
   - `GET /dashboard/forgot-password` — form for email input (use AuthLayout)
   - `POST /api/auth/forgot-password` — generate token, store in DB, send reset email
   - `GET /dashboard/reset-password?token=xxx` — form for new password
   - `POST /api/auth/reset-password` — validate token, update password hash

3. **Node auto-restart on server reboot** — Node is started manually via `/tmp/restart-node.sh`. If the cPanel server reboots, Node stops and the site goes dark. Options: cPanel cron job `@reboot`, or ask Fritz about cPanel's "Node.js" app manager if available.

#### Known Issues (Carried From Prior Wave)
- `POST /api/admin/orders` — GoDaddy API credentials not configured on server, returns "Not Found" error (non-blocking for current PayPal flow)
- GoDaddy shopper creation on register always fails non-fatally (reseller account not linked to API key)
- `thank-you.astro` still references GoDaddy provisioning — this is the old pre-PayPal purchase flow page

---

### Current Verified State (End of Wave 2)

| Component | Status |
|-----------|--------|
| `/dashboard/login` | ✅ AuthLayout, no sidebar, no phone |
| `/dashboard/register` | ✅ AuthLayout, no sidebar, form works |
| `/admin/login` | ✅ AuthLayout, no sidebar |
| Logout button | ✅ Sidebar footer, works live |
| Customer number | ✅ Topbar + sidebar, zero-padded |
| Add to Cart (all products) | ✅ Live, DB-confirmed |
| `/checkout` | ✅ 200, PayPal SDK loads |
| GoDaddy phone — public | ✅ Removed everywhere except /contact |
| GoDaddy name — customers | ✅ Removed from dashboard billing/email |
| Node on server | ✅ Running, 127.0.0.1:3001 |
| PayPal sandbox click-through | ⬜ Browser test not done — Shay's next task |
| Welcome email on register | ⬜ Not built |
| Forgot password | ⬜ Not built |
| Node auto-restart | ⬜ Not configured |
