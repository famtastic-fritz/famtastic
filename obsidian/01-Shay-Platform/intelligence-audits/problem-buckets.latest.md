---
title: problem-buckets.latest
type: note
permalink: famtastic/01-shay-platform/intelligence-audits/problem-buckets.latest
---

# Problem buckets

Generated: 2026-06-22T19:52:57.148477+00:00

## Canonical buckets
### Platform-conversation routing/context gap (`platform-conversation-context`)
- type: routing_gap
- frequency: 3
- status: open
- promotion_status: pending
- sites: site-famtastic-com, site-mbsh96reunion
- aliases: special_case_for_platfor_convos, platform convo, platform work, to_do_some_platform_work
- action: Distinguish platform/orchestration conversations from site-build conversations before classifier or gap logging writes malformed capability rows.

### Shay Desk UI friction (`shay-desk-ui-kinks`)
- type: ux_gap
- frequency: 3
- status: open
- promotion_status: pending
- sites: site-mbsh96reunion
- aliases: text-area, new line, to_work_out_some_kinks_with_you, shay desk inter face
- action: Group newline, textarea sizing, and Shay Desk interaction complaints into one UI-fix bucket.

### Video background capability (`video-background`)
- type: capability_unproven
- frequency: 2
- status: open
- promotion_status: unpromoted
- sites: site-altitude
- aliases: video_background_option, video_background_support, video background, video hero, video-hero
- action: Prove or close the existing video-hero path before opening new research/build loops.

### Billing and API-usage visibility (`billing-usage-visibility`)
- type: ops_gap
- frequency: 1
- status: open
- promotion_status: unpromoted
- sites: site-mbsh96reunion
- aliases: track billing, api use, billing
- action: Create a dedicated billing/usage tracking workstream instead of leaving it embedded in ad hoc prompts.

### Client brief mislogged as platform gap (`client-brief-mislogged-as-gap`)
- type: intake_gap
- frequency: 1
- status: open
- promotion_status: pending
- sites: site-crumb-and-crust-bakery
- aliases: family bakery, three pages, keep things simple, readable
- action: Treat client/site briefs as intake artifacts, not capability gaps, and route them into brief/build surfaces before gap logging.

### GoDaddy/cPanel access and routing (`godaddy-cpanel-access`)
- type: execution_routing_gap
- frequency: 1
- status: open
- promotion_status: unpromoted
- sites: SHAY
- aliases: godaddy, cpanel, dns, reseller, proactive-gap-escalation-order
- action: Split vendor access, DNS operations, reseller recovery, and escalation order into separate proof-backed tracks.

### Media prompt handoff routing gap (`media-prompt-handoff`)
- type: routing_gap
- frequency: 1
- status: open
- promotion_status: pending
- sites: site-famtastic-com
- aliases: leanardo ai, leonardo ai, send it to leanardo ai
- action: Route external image/video prompt handoff requests into a dedicated media workflow instead of logging them as raw malformed capability prose.

### Priority interrupt mislogged as capability gap (`priority-interrupt-mislogged-as-gap`)
- type: tasking_gap
- frequency: 1
- status: open
- promotion_status: pending
- sites: site-mbsh96reunion
- aliases: hold the discovery work, more immediate problem
- action: When Fritz reprioritizes work mid-stream, treat it as a task-routing interrupt instead of a new capability gap row.