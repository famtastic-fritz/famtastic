...
| **Built-in MEMORY.md / USER.md** | Curated agent memory | ✅ Available | live_verified | current runtime injects MEMORY/USER profile surfaces, auto-routes bulky writes to off-prompt ledgers, and now treats the always-loaded layer as an intentionally tiny under-~2k combined pointer surface instead of bulk storage | Free | Durable user preferences, environment facts, and compact always-loaded pointer context without treating prompt memory like bulk storage | Built into Hermes |
