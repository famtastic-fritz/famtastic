...
**Last updated:** 2026-06-17 (Added a versioned simple brief template under `plans/templates/` for human-readable orchestration briefs alongside the heavier plan packet surface.)
...
- **2026-06-17** — Simple brief template v1. Added `plans/templates/simple-brief-template-v1.md` as a lightweight human-brief surface that stays readable while still showing orchestration truth. The template carries `Execution` plus task-mapped `Assignments` (task → agent → model), artifact-backed `Research`/`Review`, dependency plan IDs, proof, and status, with both a blank form and a filled example. Deferred: it is documented and versioned, but not yet surfaced through `fam-hub` or enforced by a dedicated validator.
