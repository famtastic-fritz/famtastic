...
<!-- mirrored from ~/famtastic/FAMTASTIC-STATE.md -->

# FAMTASTIC-STATE.md — Canonical Project Reference

**Last updated:** 2026-06-19 (Added the Shay intelligence control-plane loop so solution-map normalization now has proof/closeout mutation, garbage-gap cleanup, pre-research guards, pass history, and an autonomous stop condition at `only_minor_left`.)
...
- **2026-06-19** — Shay intelligence control-plane loop. Added `scripts/intelligence/build_proof_closeout.py`, `cleanup_gap_rows.py`, `pre_research_guard.py`, `run_control_plane_pass.py`, and `control_plane_loop.py` so the solution-map layer can mutate proof/closeout state, classify garbage gap rows, ground fresh research against existing truth, and stop automatically when only minor findings remain. The live pass published `proof-closeout.latest.{md,json}`, `gap-cleanup.latest.{md,json}`, `control-plane-runtime-plan.md`, `control-plane-pass-history.jsonl`, and `control-plane-loop.latest.json` under `obsidian/01-Shay-Platform/intelligence-audits/`, auto-verified the repo-backed video-background capability, and reclassified the 4 previously unmatched gap rows into canonical buckets. Deferred: repo-verdict linkage is still advisory and the control-plane loop is not yet cron-driven or backlog-opening.
- **2026-06-19** — Shay solution-map normalization + promotions ledger bootstrap. Added `scripts/intelligence/normalize_solution_map.py`, which reads `~/.local/share/famtastic/gaps.jsonl` and the latest solution-map audit, groups recurring issues into canonical buckets, writes `obsidian/01-Shay-Platform/intelligence-audits/problem-buckets.latest.{md,json}`, and bootstraps `~/.local/share/famtastic/intelligence-promotions.json`. The first run created 7 canonical buckets and 5 promoted findings, finally making recurring problems like GoDaddy/cPanel routing, JJ BA brief drift, video-background proof confusion, Shay Desk UI friction, and the promotions-ledger gap itself queryable across time instead of stranded in prose. Deferred: the source gap ledger still contains malformed rows, and the normalizer does not yet auto-write bucket IDs back into `gaps.jsonl`, open backlog items, or mutate closeout truth.
- **2026-06-19** — Shay solution-map audit surface. Added `scripts/intelligence/build_solution_map.py`, which scans June commits, `CHANGELOG.md`, `SITE-LEARNINGS.md`, `FAMTASTIC-STATE.md`, `.wolf/buglog.json`, `.wolf/memory.md`, the live gap ledger, bookmark intake, curator notes, gap-research notes, and repo-intelligence artifacts into canonical markdown/JSON outputs under `obsidian/01-Shay-Platform/intelligence-audits/`. The first live run produced `solution-map-audit-2026-06-19-1240.{md,json}` plus `latest-solution-map-audit.{md,json}` and surfaced the actual anti-drift problem: truth surfaces are fragmented, repo-intelligence is mostly metadata, the gap ledger is weakly normalized, and the same themes (GoDaddy/cPanel, JJ BA, video-background proof, missing promotions ledger) are repeating across time without a canonical connector. Deferred: the audit is still heuristic/read-only and does not yet mutate normalized problem buckets, promotions, or capability closeout surfaces.
- **2026-06-19** — Shay Intelligence Control Plane CLI slice. `shay-shay` now has an executable control-plane surface instead of only doctrine docs. `shay_cli/intelligence_control_plane.py` defines module boundaries, provider/model registry, agent template registry, worker instantiation contract, memory/truth surfaces, telemetry scorecards, and route explanation. `shay_cli/intelligence_cmd.py` plus `shay_cli/main.py` now expose `shay intelligence control-plane modules|providers|templates|memory|scorecards|explain`, and `agent/process_intelligence.py` now carries the route/template/provider/review telemetry fields needed to judge routing with evidence. Verified with targeted pytest and live CLI runs. Deferred: the routing engine still uses seeded heuristics plus observed scorecards rather than a fully adaptive policy, and low-tier reviewer-lane reliability remains a real limit.
...
- Shay's new solution-map audit is still read-only and heuristic. `scripts/intelligence/build_solution_map.py` can synthesize June truth surfaces into `obsidian/01-Shay-Platform/intelligence-audits/latest-solution-map-audit.{md,json}`, but it does not yet normalize malformed gap rows, create the missing `intelligence-promotions.json` ledger, or auto-link candidate repos back into canonical capability/problem records.
- Shay's solution-map normalization and control-plane layers are now loop-safe, but they are still only partially closed-loop. `scripts/intelligence/normalize_solution_map.py`, `build_proof_closeout.py`, `cleanup_gap_rows.py`, `run_control_plane_pass.py`, and `control_plane_loop.py` now create problem buckets, mutate proof-backed rows, classify garbage gaps, and stop at `only_minor_left`; however, they still do not auto-open backlog tasks, land repo-adoption verdicts as final truth, or run on a scheduled cron lane yet.
| Gap | Priority | Detail |
|-----|----------|--------|
