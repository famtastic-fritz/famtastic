...
<!-- mirrored 2026-06-16T12:16:56.969900 from ~/famtastic/SITE-LEARNINGS.md -->
...
Operational rule: the morning briefing is not a flat task shuffle and not a cron-generated fake standup. It starts with human state, pressure/fire checks, then updates plan state, captures new obligations/blockers, classifies queue vs swarm vs direct vs human-owned work, and writes the result back into `LATEST-BRIEFING.md` plus `ACTIVE-PLAN-REGISTRY.md` when state changes. Fresh-session command expectation is now explicit: `show me my latest briefing` returns the latest briefing artifact, `show me the plans we have` returns active plans first, and `start my morning briefing` runs the interview flow.

Known gaps opened: the artifact system exists, but automatic writeback discipline is still social/process-driven rather than enforced by a dedicated command surface; the scheduled morning notification can alert Fritz, but the interactive briefing still depends on a fresh session picking up these files correctly; and briefing freshness will drift if sessions fail to update `LATEST-BRIEFING.md` after meaningful state changes.

## Shay capability/intelligence restore baseline — command surfaces recovered without touching persona/soul (2026-06-15)

In `~/famtastic/shay-shay/`, current `main` was confirmed to be a clean baseline, not the full surviving truth of preserved Shay work. The capability truth layer had been preserved on `feature/capability-truth-layer-20260614` but was absent from live `main`: `shay_cli/capabilities_cmd.py` did not exist, `tests/test_capabilities_cmd.py` did not exist, and the `shay_cli/main.py` parser wiring for the `capabilities` surface was missing. The intelligence command surface was likewise preserved off-main (`feature/shay-intelligence-layer-complete-20260614` / `preserve/main-intelligence-doctrine-20260615`) but absent from live `main`: no `shay_cli/intelligence_cmd.py`, no `shay_cli/intelligence_seed.py`, and no `intelligence` parser wiring in `shay_cli/main.py`.

Recovered into the live repo working tree on 2026-06-15: `shay_cli/capabilities_cmd.py`, `shay_cli/intelligence_cmd.py`, `shay_cli/intelligence_seed.py`, `tests/test_capabilities_cmd.py`, `tests/test_intelligence_layer.py`, plus the top-level `shay_cli/main.py` subcommand handlers and parser registration for `capabilities` and `intelligence`. This was a surgical restore only — not a whole-branch replay. `PERSONA.md` and `SOUL.md` were explicitly protected and remained unchanged during the restore.

Important classification lesson: preserved doctrine/intelligence branches are not fake residue just because they were not promoted into the final main-only cleanup baseline. Treat current `main` as canonical live baseline, treat the archive as cold storage rather than alternate truth, and re-import missing value by topic/file instead of resurrecting entire branches. Worth-reviewing-later buckets from preserved work include capability truth CLI, selective intelligence/process surfaces, and recall/session memo plumbing; do not blindly resurrect older guard-rail-heavy control surfaces or let older memory-hierarchy doctrine outrank the newer preserved live evolution.

## Shay memory source-of-truth correction — latest preserved live evolution outranks older hierarchy docs (2026-06-15)

A real drift in Shay memory analysis was closed on 2026-06-15: the 2026-06-12 hierarchy/policy docs were clean and useful, but they were no longer the highest truth once the 2026-06-14/2026-06-15 live-preservation wave landed. The older doctrine (`docs/shay-memory-hierarchy.md`, `docs/shay-memory-compaction-policy.md`, `docs/shay-session-artifact-policy.md`) describes the narrower model where `~/.shay/memories/{MEMORY,USER}.md` are the bounded hot layer, `~/.shay/state.db` is the canonical conversation-recall layer, and the Obsidian vault is a shared retrieval substrate. That is historical doctrine now, not the final read when it conflicts with newer preserved live state.

The current source-of-truth for Shay memory/runtime drift analysis is the latest preserved local evolution: in `~/famtastic/`, commits `a257227` (preserve local vault and platform work before main cleanup) and `d745ea4` (preserve local vault metadata drift before main cleanup); in `~/famtastic/shay-shay/`, commits `b30a8f5` (capability truth layer CLI), `78043aa` (intelligence layer command surface), `bde9b62` (surface capability status in routing output), and `393b5ca` (preserve local intelligence and doctrine work). Those commits preserve the better live model: the vault-backed intelligence fabric is real operating state, not just secondary retrieval. That includes `obsidian/Shay-Memory/reflections/{episodic,reflective,semantic}/`, doctrine artifacts, capability/intelligence truth surfaces, repo-doc mirrors, operational/research layers, hot-context pointers, and meaningful metadata/frontmatter drift itself.

Practical rule: when assessing Shay memory architecture or cleanup risk, prefer the newest preserved live evolution over older neat hierarchy docs. `MEMORY.md`, `USER.md`, and `state.db` still matter, but they no longer fully define canonical memory reality by themselves. If old doctrine and recent preserved live state disagree, classify the old doctrine as superseded and treat the newer 2026-06-14/2026-06-15 vault/intelligence fabric as the current truth.

## FAMtastic Hosting — canonical repo + GoDaddy deploy preflight (2026-06-11)

The canonical local repo for `famtastichosting.com` is `~/famtastic/famtastic-sites/famtastic-hosting/` tracking `git@github.com:famtastic-fritz/famtastic-hosting.git`. A duplicate local checkout existed at `~/famtastic/famtastic-hosting/`; that duplicate had drifted into deploy confusion while the canonical repo under `famtastic-sites/` was already clean and synced. Lesson: before any GoDaddy/cPanel deploy, prove repo truth first — `pwd`, `git remote -v`, `git status --short --branch`, `git fetch origin`, and `git rev-list --left-right --count origin/main...HEAD`.
