...
### 1.7 Hi-Tide Harry three-tier asset system

Fritz locked a three-tier Harry system after the Scene Marker / Info Panel interview:

1. **Ultimate Photoreal Hero Harry** — ultra-photoreal F1-style Harry for page hero moments. Each interior hero should consider Harry as part of the hero concept, doing something that fits the page theme. He may be an independent transparent layer when placement/control matters, or part of a generated hero still only when that is the right visual choice.
2. **Normal Photoreal Chatbot Harry** — transparent-background head-and-shoulders / assistant pose used in the layered chatbot avatar. This is separate from hero Harry and must support a clean alpha matte.
3. **Regular Hi-Tide Harry Content Images** — supporting Harry images for cards, Note-from-Usher slides, form success states, explainers, and smaller content moments.

This tiering prevents one Harry asset from being stretched across incompatible jobs. Hero Harry sells the reel. Chatbot Harry guides the visitor. Content Harry supports page moments.

### 1.8 Cinematic-avatar as hero architecture at thumbnail scale

The chatbot avatar is not a generic AI bubble. It is the same layered DOM architecture at 80–120px scale:
...
Frame style is locked to the **marquee bulb ring**. The bulb chase doubles as status: idle pulse, listening/thinking chase, speaking glow. This proves the layered primitive is canvas-agnostic: full hero and chatbot avatar use the same playbook at different scale.

---

## 2. Reel-by-Reel Scene Plan

Seven reels. The audience moves through them in order: Home → RSVP → Tickets → Through-Years → Memorial → Capsule → Playlist → (loops to Home). Each interior reel inherits the universal page flow (Impact Hero → mini-marquee Scene Marker → Note from Usher / configurable info panel → page content → Up Next → Footer) and contributes one cinematic hero. Home is the opening act and may omit the explicit scene marker.
...
| 2 | `.character` | Tier 1 Ultimate Photoreal Hero Harry optional/controlled | Default can remain seat-led, but the new rule is to evaluate a page-theme Harry layer for every interior hero; if Harry weakens the “seat waiting for you” message, keep him out of the hero and use him in the Note/Info Panel instead |
