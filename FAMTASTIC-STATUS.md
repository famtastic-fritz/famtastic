...
|---|---|---|
| `studio-workbench-foundation` | Active parent plan | Rebuild Workbench against the frozen workspace contract and wire Plan mode to real registry/task/run/proof data. |
...
1. P0: Rebuild Workbench against `docs/STUDIO-UI-FOUNDATION.md`.
2. P0: Wire Workbench Plan mode to real registry/task/run/proof data.
3. P0: Use Playwright and actual Shay/Shay-Shay calls for Studio behavior proof.
4. P0/P1: Declare BuildIntent v2 current and instrument `parallelBuild()`.
5. P1/P2: Start workflow-as-data phase 1, then build pipeline visualizer inspect + trace + propose.
6. P0/P1: Split MBSH site work from reusable platform gaps.
