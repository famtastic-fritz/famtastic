// AUDIT skill: analyze the current NCS7 site and the business, surface gaps the
// modernization will close. Produces a real audit artifact.
...
export async function run(task, { llm }) {
  ...
  const think = await llm.complete({
    system: 'You are a conversion + product auditor for legacy publishing sites.',
    prompt: `Audit ${target}: a 30-year-old site selling viewable/downloadable CAD-standard PDFs. ` +
      `Find UX, conversion, content-ops and product gaps a modern React + CMS + 3D rebuild would close.`,
    model: task._model.id,
  });

  const md = `# NCS7 — Current-State Audit

> Audit of ${target} (the National CAD Standard v7 sales site). The product today
...
`;
  ...
  const artifact = writeArtifact('docs/ncs7/02-audit.md', md);
  return {
    summary: 'Current-state audit of NCS7 with prioritized gaps',
    artifacts: [artifact],
    usage: think.usage,
    metrics: { gaps: 7 },
  };
