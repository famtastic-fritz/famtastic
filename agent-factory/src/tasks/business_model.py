"""The proof task: a complete go-to-market + sales pipeline for selling
FAMtastic Designs as a product (internal use).

This is the deliverable a worker agent produces when it claims a
`business_model` task. The LLM call is exercised for routing/cost realism, but
the structured business artifact is authored here so the offline run yields a
genuinely useful document rather than stub text.

Every assumption is logged inline (marked **ASSUMPTION**) and re-collected in
SUMMARY.md at the end of the run.
"""
from __future__ import annotations


def build(payload: dict) -> str:
    product = payload.get("product", "FAMtastic Designs")
    paypal = payload.get("paypal_account", "<PayPal Business — provided at activation>")
    domain = payload.get("godaddy_domain", "famtasticdesigns.com")
    from_email = payload.get("outreach_from", f"hello@{domain}")

    return f"""# {product} — Internal Business Model & End-to-End Sales Pipeline
...
**{product}** productizes the FAMtastic site factory: confidently-different,
...
- **Proof-led content:** publish the actual factory builds as a portfolio at
  `{domain}/work` (one case study per site already in `famtastic-dna.md`).
...
- **Sequence per campaign:**
  - Day 0: cold email #1 (teardown offer) from `{from_email}`.
...
- **Sending domain:** `{domain}` via **GoDaddy**. **ASSUMPTION:** a dedicated
  outreach subdomain (e.g., `mail.{domain}` or a separate `.com`) so the primary
  domain reputation is protected.
- **Mailbox / identity:** custom address `{from_email}` (GoDaddy custom email —
...
- **Sending capability:** GoDaddy email (SMTP `smtpout.secureserver.net`) is
  ...
  cold-email ESP (Instantly/Smartlead-style) while keeping the same `{domain}`
  identity. The pipeline treats "send" as a pluggable interface so this swap is
  config-only.
- **Domain warmup:** 2–3 weeks ramp, SPF/DKIM/DMARC set on `{domain}` before any
...
**Account:** {paypal} — **ASSUMPTION:** PayPal Business credentials are provided
...
1. **Week 1:** stand up `{domain}`, custom mailbox, SPF/DKIM/DMARC, PayPal
