...
const needsYou = plans.filter((p) => p.needsYou)
  .sort((a, b) => (b.profit + b.autonomy) - (a.profit + a.autonomy));
...
fs.writeFileSync(path.join(OUT_DIR, 'state.json'), JSON.stringify(snapshot, null, 2));
...
fs.writeFileSync(path.join(OUT_DIR, 'briefing.md'), briefing);
...
function planCard(p) {
  ...
  return `<div class="card" data-needs="${p.needsYou}">
    <div class="card-head">
...
<style>
  ...
  *{box-sizing:border-box}
  ...
  .kpi.alert b{color:#f5b342}.kpi.danger b{color:#ef4444}
...
<div class="wrap">
  ...
  <section>
    ...
    <div class="cards">${[...plans].sort((a,b)=>(Number(b.needsYou)-Number(a.needsYou))||(b.profit-a.profit)).map(planCard).join('')}</div>
  </section>
