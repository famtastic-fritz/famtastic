...
const STEERING_FILE = path.join(__dirname, '..', '..', 'obsidian', 'Shay-Memory', '_system', 'runtime', 'proactive', 'behavior-steering.json');
...
function buildMemoryBlock(ctx = {}) {
  ...
  try {
    if (fs.existsSync(STEERING_FILE)) {
      const steering = JSON.parse(fs.readFileSync(STEERING_FILE, 'utf8'));
      lines.push('### Response steering (behavioral overlay)');
      lines.push(`- response_mode: ${steering.response_mode || 'normal'}`);
      lines.push(`- ask_threshold: ${steering.ask_threshold || 'medium'}`);
      lines.push(`- pushback_level: ${steering.pushback_level || 'direct'}`);
      lines.push(`- prioritization_bias: ${steering.prioritization_bias || 'execution'}`);
      if (steering.must_not_flatten) lines.push('- must_not_flatten_sensitive_context: true');
      lines.push('');
    }
  } catch (_) {}
